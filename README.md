...
The generated dataset is made of sub-directories for all the chips, under which the data for each day is a separate folder.

To prepare training data, use the [prepare script](./prepare.py) to divide data for training, validation and test sets, in the ratio of 8:1:1.

If using the [OpenSTL](https://github.com/chengtan9907/OpenSTL) framework, here's the [script](./train.py) to set up the data loader and training configs. It's aimed for using the COnvLSTM model, so you may have to adjust the params accordingly.
